...
system.time(thinned.ints <- collect.digit.skeletons(digits[rand.idcs,]))
```
...
*ADD ME: Describe path generating algorithm*
...
Collect apriori probabilities for test set:
...
Predict a test digit using Bayes Rule: For each test digit, calculate the posterior probability of each digit in the set {0,...,9} and predict which ever digit as the greatest posterior probability.

$$P(digit = 0| \text{path data}) = P(\text{path data} | digit = 0) P(digit = 0)$$
